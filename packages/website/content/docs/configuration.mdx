...
| Provider | Example model                | Required env var |
| -------- | ---------------------------- | ---------------- |
| `openai` | `text-embedding-3-large`     | `OPENAI_API_KEY` |
| `gemini` | `gemini-embedding-2-preview` | `GEMINI_API_KEY` |
| `cohere` | `embed-v4.0`                 | `COHERE_API_KEY` |
| `voyage` | `voyage-3`                   | `VOYAGE_API_KEY` |
| `ollama` | `nomic-embed-text`           | _(none — local)_ |

Set `embeddings.provider`, `embeddings.model`, and `embeddings.dimensions` to
match your chosen provider, and set the corresponding API key. The Cloudflare
worker reads the same provider via the `EMBEDDING_PROVIDER` / `EMBEDDING_MODEL`
/ `EMBEDDING_DIMENSIONS` variables — keep them in sync.
