...
Crawl a live documentation website and index its pages. Page discovery prefers
`sitemap.xml`, falling back to a same-origin breadth-first crawl. Use the `html`
parser, which strips boilerplate (nav/header/footer) and converts to Markdown.
...
sources:
  - name: docs-site
    ...
    parser: html
    baseUrl: https://docs.example.com
    sitemap: https://docs.example.com/sitemap.xml # optional, defaults to <origin>/sitemap.xml
    maxPages: 100 # optional, default 100
    crawlDepth: 3 # optional, default 3 (only used when crawling without a sitemap)
```
...
| `html`     | Raw `.html`/`.htm` or crawled docs | Strips boilerplate, converts to Markdown    |
