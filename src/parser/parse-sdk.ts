...
const SKIP_DIRS = new Set(['.git', '.github', 'node_modules', 'vendor', 'dist', 'build', 'coverage', '__pycache__', '.venv', 'venv']);
const SKIP_FILES = new Set(['security.md', 'code_of_conduct.md', 'issue_template.md', 'pull_request_template.md', 'license.md', 'contributing.md', 'api.md']);
const SKIP_INTERNAL_DIRS = new Set(['src/internal', 'src/core', 'internal', 'test', 'tests', '__tests__', 'spec']);
...
function parseDocFile(
  ...
  const contextualRepoName = packageName && packageName !== repoInfo.name
...
async function main() {
  ...
  for (const repo of REPOSITORIES) {
    ...
    try {
      ...
      stats.push({ repo: repoInfo.fullName, language: repo.language, files: docFiles.length, chunks: repoChunks });
