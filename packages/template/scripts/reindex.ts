  ...
  if (!args.dryRun && allChunks.length > 0 && store && embedClient) {
    ...
    if (incremental) {
      ...
      if (diff.toDelete.length > 0) {
        ...
        // Route the delete through the vector-store abstraction so it works for
        // any backend (Pinecone, Qdrant, ...), not just raw Pinecone.
        await store.delete(diff.toDelete);
        console.log('   ✅ Deleted');
      }
    }
