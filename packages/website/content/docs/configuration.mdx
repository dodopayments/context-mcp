...
**Cost note:** `text-embedding-3-large` costs ~$0.13 per 1M tokens. Incremental reindexing avoids re-embedding unchanged chunks, so scheduled runs usually cost much less than a full rebuild.
...
```yaml
reindex:
  clearBeforeReindex: false # Incremental by default
  batchSize: 100 # Vectors uploaded per batch
```

`npm run reindex` stores chunk hashes in `data/index-manifest.json`, compares the latest parse with that manifest, then reports `added`, `updated`, `unchanged`, and `deleted` counts. Added and updated chunks are embedded and upserted; unchanged chunks are skipped; deleted chunks are removed from Pinecone by ID.

Set `clearBeforeReindex: true` or run `npm run reindex:full` to force a complete rebuild.
