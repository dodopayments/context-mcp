...
<Tabs items={['OpenAI', 'Google Gemini', 'Cohere', 'Voyage AI', 'Ollama']}>
...
Run embeddings fully offline with a local [Ollama](https://ollama.com) server. No API key, no per-token cost.

Pull the model first: `ollama pull nomic-embed-text`.

<Callout type="warn" title="Worker note">
  The Cloudflare worker generates query embeddings at request
  ...
  worker or run the self-hosted Node server.
</Callout>
...
<Callout title="Note">
  The `dimensions` value must match the model you choose, and the
...
<Tabs items={['GitHub Repository', 'GitLab Repository']}>

<Tab value="GitHub Repository">
