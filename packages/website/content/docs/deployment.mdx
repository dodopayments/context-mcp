...
`npm run reindex` is incremental. Cache `data/index-manifest.json` between workflow runs to avoid re-embedding unchanged chunks on fresh GitHub-hosted runners.
...
For scheduled workflows, add an Actions cache step for the generated manifest:

```yaml
- name: Restore index manifest
  uses: actions/cache/restore@v4
  with:
    path: data/index-manifest.json
    key: index-manifest-${{ github.run_id }}
    restore-keys: |
      index-manifest-
